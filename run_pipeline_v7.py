...
def fetch_article_name(url, timeout=15):
    ...
    try:
        with urllib.request.urlopen(urllib.request.Request(url, headers=UA), timeout=timeout) as r:
            raw = r.read(600000)                       # first ~600KB: head + body lede
            charset = r.headers.get_content_charset() or "utf-8"
        host = urllib.parse.urlparse(url).netloc.lower()
        text = raw.decode(charset, "replace")
    except Exception:
        return None
    # 1) the article's own headline tags
    for rx in ARTICLE_TITLE_RES:
        for m in rx.finditer(text):
            ...
            if cand:
                ...
                if nm and _plausible_school_name(nm, host):
                    return nm
    # 2) the article body: most-repeated named school wins
    body = _clean_html_text(SCRIPT_STYLE_RE.sub(' ', text))
    counts = {}
...
def main():
    ...
    for it in raw:
        ...
        # Headline named no school -> look inside the article itself.
        if not extracted_name:
            body_name = fetch_article_name(real_link)
            if body_name:
